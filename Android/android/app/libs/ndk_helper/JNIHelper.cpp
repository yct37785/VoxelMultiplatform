...
//---------------------------------------------------------------------------
// readFile and returns the buffer
//---------------------------------------------------------------------------
char* JNIHelper::ReadFile_toBuffer(const char* fileName)
{
    if (activity_ == NULL) {
        LOGI(
                "JNIHelper has not been initialized.Call init() to initialize the "
    ...
    if (f) {
       ...
       //                    std::istreambuf_iterator<char>());
        ...
        char* return_buf = new char[fileSize];
        f.read(return_buf, fileSize);
        ...
        char* data = (char*)AAsset_getBuffer(assetFile);
        int32_t size = AAsset_getLength(assetFile);
        if (data == NULL) {
            AAsset_close(assetFile);
